...
class Diag extends Component {
    ...
    getScore() {
        let score = -1;
        for(let values of this.props.valuesRange){
            score++;
    ...
    getPolygon(points, color) {
        ...
        elem.setAttribute("style", "fill-opacity: 1;fill: " + color + ";stroke: #000000;" + (this.state.shadow ? "filter:url(#fs)" : ""));
    ...
    createSVG() {
        ...
        var blocHeight = (this.state.height - ((countElem + 1) * this.state.pad)) / countElem;
        ...
        for (var i = 0; i < countElem; i++) {
            ...
            if (score === i) {
                ...
                var scorePath = "M " + sx1 + " " + sy1 +
                    ...
                    "L" + sx2 + " " + sy2 +
                    "L" + sx1 + " " + sy2 + " Z";
                svg.appendChild(this.getPath(scorePath, {
                    'stroke': '#5b5b5b',
                    'strokeWidth': 1,
                    'fill': "#ffffff",
                    'fillOpacity': 0.8
...
export function DPE(props) {
    ...
    return <Diag
        {...props} valuesRange={valuesRange} shape={"sharp"}
    />;
}
...
export function GES(props) {
    ...
    return <Diag
        {...props} valuesRange={valuesRange} shape={"flat"}
    />;
}